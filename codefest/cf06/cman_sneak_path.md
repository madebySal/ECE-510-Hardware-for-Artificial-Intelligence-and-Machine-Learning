...
In a resistive crossbar performing Matrix-Vector Multiplication (MVM), each column output current should represent only the dot product of the input voltage vector with that column's programmed conductance weights. but when unselected rows and columns are left floating, they settle at intermediate voltages through resistive coupling (V_row1 = 0.4 V, V_col1 = 0.6 V here), allowing parasitic current to flow through off-state cells and corrupt the intended reading. This sneak-path error scales with array size: in an N×N crossbar each off-state cell can participate in O(N²) parasitic loops, so the spurious current grows with array dimensions and makes large-scale analog MVM increasingly inaccurate. Mitigation strategies such as series selector devices (diodes, OTS selectors, or access transistors) or active column clamping are therefore required in practical crossbar arrays.
